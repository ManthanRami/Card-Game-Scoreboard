...
describe('Kachufolio Game Logic', () => {
  ...
  describe('generateGameRounds', () => {
    it('should return an empty array for less than 2 players', () => {
      expect(generateGameRounds(1)).toEqual([]);
    });

    it('should generate rounds correctly for 4 players', () => {
      ...
      expect(generateGameRounds(4).length).toBe(39);
    });

    it('should generate rounds correctly for 6 players', () => {
        // 52 cards / 6 players = 8 max cards
        const max = 8;
        const up = Array.from({ length: max }, (_, i) => i + 1);
        const down = Array.from({ length: max }, (_, i) => max - i);
        const expected = [...up, ...down, ...up];
        expect(generateGameRounds(6)).toEqual(expected);
        expect(generateGameRounds(6).length).toBe(24);
    });

    it('should return an empty array if max cards is less than 1', () => {
      expect(generateGameRounds(53)).toEqual([]);
    });
  });
});
